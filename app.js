...
function renderListe(ziel, slug) {
  if (LISTEN[slug] && LISTEN[slug].typ === 'wandern-rubrik') {
    renderWandernRubrik(ziel, slug, LISTEN[slug]);
    return;
  }
  ...
  if (teile[0] === 'tourismus' && teile[1] === 'wandern' && teile[2]) {
    // Alte Tour-Reihen-Sub-Pages (westerwaldsteig, druidensteig, ...) gibt es
    // nicht mehr — alles laeuft jetzt ueber die einheitliche Wandern-Rubrik
    // mit Filter-Pillen. Anfrage darauf umleiten.
    window.location.hash = 'kategorie/tourismus-wandern';
    return;
  }
...
function renderDetail(ziel, typ, schluessel) {
  // Spezialfall: DataHub-Wandertour (neue Rubrik, eigenes Daten-Schema)
  if (typ === 'wandertour') {
    renderWandertourDetail(ziel, schluessel);
    return;
  }
...
// ════════════════════════════════════════════════════════════════
// WANDERN-RUBRIK (DataHub-basiert, neu ab 06/2026)
// ════════════════════════════════════════════════════════════════
// Quelle: window.DATA_WANDERN_ALLE (aus wandertouren-datahub.js)
// Schema pro Eintrag: id, slug, titel, tourReihe, laengeKm, hoehenmeterAuf/Ab,
// dauerMin, minHoehe, maxHoehe, schwierigkeit (0-5), istRundweg,
// beschreibung, text, anreise, ausruestung, hinweise, empfehlung,
// hinweistext, bild, thumb, bildLizenz, bildUrheber,
// startLat, startLng, track (Array von Segmenten mit [lng,lat,h]-Tripeln),
// url.

var WANDER_FILTER = { reihe:'alle', schwierigkeit:'alle', laenge:'alle', form:'alle' };

function wandernFilterUI() {
  function pill(group, val, label) {
    var aktiv = WANDER_FILTER[group] === val;
    return '<button class="filter-pill' + (aktiv ? ' aktiv' : '') + '" '
      + 'onclick="setzeWanderFilter(\'' + group + '\',\'' + val + '\')">' + label + '</button>';
  }
  var html = '<div class="filter-leiste termine-filter">';
  html += '<div class="filter-gruppe"><span class="filter-label">🥾 Tour-Reihe:</span>'
    + pill('reihe','alle','Alle')
    + pill('reihe','Westerwaldsteig','Westerwaldsteig')
    + pill('reihe','Wiedweg','Wiedweg')
    + pill('reihe','Druidensteig','Druidensteig')
    + pill('reihe','Wäller Touren','Wäller Touren')
    + pill('reihe','Kleine Wäller','Kleine Wäller')
    + pill('reihe','Einzeltour','Einzeltouren')
    + '</div>';
  html += '<div class="filter-gruppe"><span class="filter-label">📊 Schwierigkeit:</span>'
    + pill('schwierigkeit','alle','Alle')
    + pill('schwierigkeit','leicht','Leicht')
    + pill('schwierigkeit','mittel','Mittel')
    + pill('schwierigkeit','schwer','Schwer')
    + '</div>';
  html += '<div class="filter-gruppe"><span class="filter-label">📏 Länge:</span>'
    + pill('laenge','alle','Alle')
    + pill('laenge','kurz','&lt; 5 km')
    + pill('laenge','mittel','5–15 km')
    + pill('laenge','lang','15–30 km')
    + pill('laenge','sehrlang','&gt; 30 km')
    + '</div>';
  html += '<div class="filter-gruppe"><span class="filter-label">🔄 Form:</span>'
    + pill('form','alle','Alle')
    + pill('form','rund','Rundweg')
    + pill('form','strecke','Streckentour')
    + '</div>';
  html += '</div>';
  return html;
}

function setzeWanderFilter(group, val) {
  WANDER_FILTER[group] = val;
  var wrap = document.getElementById('filter-leiste-wrapper');
  if (wrap) wrap.innerHTML = wandernFilterUI();
  var liste = document.getElementById('wander-liste');
  if (liste) liste.innerHTML = baueWanderListe();
  aktualisiereWanderTreffer();
}

function wanderFilterAnwenden(items) {
  return items.filter(function(t) {
    // Tour-Reihe
    if (WANDER_FILTER.reihe !== 'alle' && t.tourReihe !== WANDER_FILTER.reihe) return false;
    // Schwierigkeit
    if (WANDER_FILTER.schwierigkeit !== 'alle') {
      var s = t.schwierigkeit || 0;
      if (WANDER_FILTER.schwierigkeit === 'leicht'  && !(s >= 1 && s <= 2)) return false;
      if (WANDER_FILTER.schwierigkeit === 'mittel'  && s !== 3)             return false;
      if (WANDER_FILTER.schwierigkeit === 'schwer'  && !(s >= 4 && s <= 5)) return false;
    }
    // Länge
    if (WANDER_FILTER.laenge !== 'alle') {
      var km = t.laengeKm || 0;
      if (WANDER_FILTER.laenge === 'kurz'     && !(km < 5))               return false;
      if (WANDER_FILTER.laenge === 'mittel'   && !(km >= 5 && km < 15))   return false;
      if (WANDER_FILTER.laenge === 'lang'     && !(km >= 15 && km < 30))  return false;
      if (WANDER_FILTER.laenge === 'sehrlang' && !(km >= 30))             return false;
    }
    // Form
    if (WANDER_FILTER.form === 'rund'    && !t.istRundweg) return false;
    if (WANDER_FILTER.form === 'strecke' &&  t.istRundweg) return false;
    return true;
  });
}

function aktualisiereWanderTreffer() {
  var alle = window.DATA_WANDERN_ALLE || [];
  var gefiltert = wanderFilterAnwenden(alle);
  var el = document.getElementById('filter-treffer');
  if (el) el.innerHTML = '<strong>' + gefiltert.length + '</strong> von <strong>' + alle.length + '</strong> Wandertouren';
}

function schwierigkeitsBadge(s) {
  if (!s) return '';
  var lbl = ['', 'Sehr leicht', 'Leicht', 'Mittel', 'Schwer', 'Sehr schwer'][s] || ('Stufe ' + s);
  var farbe = s <= 2 ? 'diff-leicht-bg' : (s === 3 ? 'diff-mittel-bg' : 'diff-schwer-bg');
  return '<span class="diff-pill ' + farbe + '">' + escapeHtml(lbl) + '</span>';
}

function baueWanderListe() {
  var alle = window.DATA_WANDERN_ALLE || [];
  var gefiltert = wanderFilterAnwenden(alle);
  if (!gefiltert.length) {
    return '<div class="hinweis">Keine Tour entspricht den aktuellen Filtern.</div>';
  }
  // Sortierung: Tour-Reihe, dann Titel
  gefiltert.sort(function(a,b) {
    if (a.tourReihe !== b.tourReihe) return a.tourReihe < b.tourReihe ? -1 : 1;
    var ta = (a.titel || '').toLowerCase(), tb = (b.titel || '').toLowerCase();
    return ta < tb ? -1 : (ta > tb ? 1 : 0);
  });
  var html = '<div class="termine-liste">';
  gefiltert.forEach(function(t) {
    var pills = '';
    pills += '<span class="diff-pill">📏 ' + t.laengeKm.toFixed(1).replace('.', ',') + ' km</span>';
    if (t.hoehenmeterAuf) pills += '<span class="diff-pill">⛰️ ' + t.hoehenmeterAuf + ' hm</span>';
    if (t.dauerMin)       pills += '<span class="diff-pill">⏱ ' + Math.round(t.dauerMin/60*10)/10 + ' Std.</span>';
    pills += '<span class="diff-pill">' + (t.istRundweg ? '🔄 Rundweg' : '➡ Streckentour') + '</span>';
    if (t.schwierigkeit)  pills += schwierigkeitsBadge(t.schwierigkeit);
    if (t.tourReihe && t.tourReihe !== 'Einzeltour')
      pills += '<span class="diff-pill">🥾 ' + escapeHtml(t.tourReihe) + '</span>';

    var bildHtml = '';
    if (t.thumb || t.bild) {
      var src = t.thumb || t.bild;
      bildHtml = '<div class="termin-bild"><img loading="lazy" src="' + escapeHtml(src) + '" alt=""></div>';
    }

    var beschreibung = (t.beschreibung || '').slice(0, 200);
    if ((t.beschreibung || '').length > 200) beschreibung += '…';

    html += '<button class="termin-eintrag" onclick="navigateTo(\'detail/wandertour/' + escapeHtml(t.slug || t.id) + '\')">'
      + bildHtml
      + '<div class="termin-inhalt">'
      +   '<div class="termin-titel">' + escapeHtml(t.titel) + '</div>'
      +   '<div class="termin-pills">' + pills + '</div>'
      +   (beschreibung ? '<div class="termin-beschreibung">' + escapeHtml(beschreibung) + '</div>' : '')
      + '</div>'
      + '<div class="subkat-pfeil">&rsaquo;</div>'
      + '</button>';
  });
...
function renderWandernRubrik(ziel, slug, info) {
  var alle = window.DATA_WANDERN_ALLE || [];
  if (!alle.length) {
    ziel.innerHTML =
      '<div class="sticky-region">'
      + navBar(info.zurueck, info.breadcrumb)
      + intro(info.titel, info.untertitel)
      + '</div>'
      + '<div class="hinweis">Wandertouren-Daten noch nicht geladen.</div>'
      + '<div class="spacer"></div>';
    return;
  }
  WANDER_FILTER = { reihe:'alle', schwierigkeit:'alle', laenge:'alle', form:'alle' };
  ziel.innerHTML =
    '<div class="sticky-region">'
    + navBar(info.zurueck, info.breadcrumb)
    + intro(info.titel, info.untertitel)
    + '<div id="filter-leiste-wrapper">' + wandernFilterUI() + '</div>'
    + '<div id="filter-treffer" class="filter-treffer"></div>'
    + '</div>'
    + '<div id="wander-liste">' + baueWanderListe() + '</div>'
    + '<div class="spacer"></div>';
  aktualisiereWanderTreffer();
}

// ─── DETAIL-PAGE EINER EINZELNEN WANDERTOUR ──────────────────────
// Sucht die Tour ueber slug ODER id im DATA_WANDERN_ALLE-Array
function findeWandertour(schluessel) {
  var alle = window.DATA_WANDERN_ALLE || [];
  var s = (schluessel || '').toLowerCase();
  for (var i = 0; i < alle.length; i++) {
    var t = alle[i];
    if ((t.slug || '').toLowerCase() === s) return t;
    if ((t.id   || '').toLowerCase() === s) return t;
  }
  return null;
}

function renderWandertourDetail(ziel, schluessel) {
  var t = findeWandertour(schluessel);
  if (!t) {
    ziel.innerHTML = navBar('home','') + intro('Nicht gefunden','')
      + '<div class="hinweis">Wandertour nicht verfuegbar.</div>';
    return;
  }
  var mapId = 'wandertour-karte-' + Math.random().toString(36).slice(2);

  // Bild oben
  var bildHtml = '';
  if (t.bild) {
    bildHtml = '<div class="termin-hero"><img loading="lazy" src="' + escapeHtml(t.bild) + '" alt="' + escapeHtml(t.titel) + '">';
    if (t.bildUrheber) {
      bildHtml += '<div class="termin-bild-credit">Foto: ' + escapeHtml(t.bildUrheber)
        + (t.bildLizenz ? ' (<a href="' + escapeHtml(t.bildLizenz) + '" target="_blank" rel="noopener">Lizenz</a>)' : '')
        + '</div>';
    }
    bildHtml += '</div>';
  }
  ...
  // Metadaten-Tabelle
  var metaRows = [];
  metaRows.push(['Länge',          t.laengeKm.toFixed(1).replace('.',',') + ' km']);
  if (t.hoehenmeterAuf)  metaRows.push(['Aufstieg',  t.hoehenmeterAuf + ' Hm']);
  if (t.hoehenmeterAb)   metaRows.push(['Abstieg',   t.hoehenmeterAb + ' Hm']);
  if (t.dauerMin)        metaRows.push(['Dauer',     Math.floor(t.dauerMin/60) + ' Std. ' + (t.dauerMin%60) + ' Min.']);
  if (t.minHoehe || t.maxHoehe) metaRows.push(['Höhe', (t.minHoehe || '?') + ' – ' + (t.maxHoehe || '?') + ' m ü. NN']);
  metaRows.push(['Form', t.istRundweg ? '🔄 Rundweg' : '➡ Streckentour']);
  if (t.schwierigkeit)   metaRows.push(['Schwierigkeit', schwierigkeitsBadge(t.schwierigkeit)]);
  if (t.tourReihe && t.tourReihe !== 'Einzeltour') metaRows.push(['Tour-Reihe', escapeHtml(t.tourReihe)]);

  var metaHtml = '<div class="termin-meta-grid">';
  metaRows.forEach(function(r) {
    metaHtml += '<div class="meta-zelle"><span class="meta-label">' + r[0] + ':</span> '
      + '<span class="meta-wert">' + r[1] + '</span></div>';
  });
  metaHtml += '</div>';
  ...
  function block(titel, inhalt) {
    if (!inhalt) return '';
    return '<div class="info-block"><h3>' + titel + '</h3><div class="info-inhalt">'
      + escapeHtml(inhalt).replace(/\n+/g, '<br>') + '</div></div>';
  }
  textBloecke += block('Beschreibung',      t.beschreibung);
  textBloecke += block('Über die Tour',     t.text);
  textBloecke += block('Anreise / ÖPNV',    t.anreise);
  textBloecke += block('Ausrüstung',        t.ausruestung);
  textBloecke += block('Hinweise / Sicherheit', t.hinweise);
  textBloecke += block('Empfehlung',        t.empfehlung);
  if (t.hinweistext && t.hinweistext !== t.text) {
    textBloecke += block('Weitere Infos',   t.hinweistext);
  }

  // Externer Link
  var extLink = t.url
    ? '<div class="info-block"><a class="btn-action outline" href="' + escapeHtml(t.url) + '" target="_blank" rel="noopener">📍 Im Tourenplaner Rheinland-Pfalz öffnen</a></div>'
    : '';

  ziel.innerHTML =
    '<div class="sticky-region">'
    + navBar('liste/tourismus-wandern', 'Wandern › <strong>' + escapeHtml(t.titel) + '</strong>')
    + intro(t.titel, '')
    + '</div>'
    + bildHtml
    + metaHtml
    ...
    +   '<div class="karte-lade-hinweis" id="' + mapId + '-lade">Karte wird geladen …</div>'
    + '</div>'
    + '<div class="karte-meta"><p>Tour-Verlauf · Daten: DataHub RLP · Kartendaten: © OpenStreetMap-Mitwirkende</p></div>'
    + textBloecke
    + extLink
    + '<div class="spacer"></div>';
  ...
  ladeKartenPlugins().then(function() {
    initWandertourKarte(mapId, t);
  }).catch(function(err) {
    var laden = document.getElementById(mapId + '-lade');
    if (laden) laden.innerHTML = 'Karte konnte nicht geladen werden.';
    console.warn('[Wandertour-Karte] Fehler beim Laden der Karten-Plugins:', err);
  });
}

// ─── KARTE FUER EINE WANDERTOUR (mit inline GeoJSON-Track) ────────
// Konvertiert das in-Schema-Format track=[ [[lng,lat,h], ...], ...]
// in Leaflet-Polylines (Leaflet erwartet [lat,lng]) und zeichnet sie ein.
function initWandertourKarte(mapId, t) {
  var elemKarte = document.getElementById(mapId);
  var elemLaden = document.getElementById(mapId + '-lade');
  if (!elemKarte) return;

  if (!t.track || !t.track.length) {
    if (elemLaden) elemLaden.innerHTML = 'Für diese Tour ist kein Streckenverlauf hinterlegt.';
    return;
  }
  ...
  // Track-Segmente einzeichnen
  var alleBounds = null;
  var farbe = '#cc0033';
  t.track.forEach(function(seg) {
    if (!seg || !seg.length) return;
    var latlngs = seg.map(function(p) { return [p[1], p[0]]; }); // [lat,lng]
    var line = L.polyline(latlngs, { color: farbe, weight: 5, opacity: 0.85 }).addTo(map);
  ...
  if (t.startLat && t.startLng) {
    L.marker([t.startLat, t.startLng]).addTo(map).bindPopup('<strong>Start</strong><br>' + escapeHtml(t.titel));
  }

  // Karte auf gesamte Tour zoomen
  if (alleBounds && alleBounds.isValid()) {
    map.fitBounds(alleBounds, { padding: [30, 30] });
  } else if (t.startLat && t.startLng) {
    map.setView([t.startLat, t.startLng], 14);
  }

  if (elemLaden) elemLaden.style.display = 'none';

  // Standort-Banner einblenden (analog zu der anderen Karten-Variante)
